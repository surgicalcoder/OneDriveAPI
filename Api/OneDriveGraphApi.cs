...
public class OneDriveGraphApi : OneDriveApi
{
    ...
    /// <summary>
    /// Defines the maximum allowed file size that can be used for basic uploads. Should be set 4 MB as described in the
    /// API documentation at https://developer.microsoft.com/en-us/graph/docs/api-reference/v1.0/api/item_uploadcontent
    /// </summary>
    public new static long MaximumBasicFileUploadSizeInBytes = 4 * 1024;

    /// <summary>
    /// Size of the chunks to upload when using the resumable upload method. Must be a multiple of 327680 bytes. See
    /// https://developer.microsoft.com/en-us/graph/docs/api-reference/v1.0/api/driveitem_createuploadsession#best-practices
    /// </summary>
    ...
    /// </summary>
    public string[] DefaultScopes => ["offline_access", "files.readwrite.all"];
    ...
    /// </summary>
    /// <param name="item">The OneDrive item to add the permission to</param>
    /// <param name="requireSignin">
    /// Boolean to indicate if the user has to sign in before being able to access the OneDrive
    /// item
    /// </param>
    ...
    /// <param name="sendInvitation">
    /// Send an e-mail to the invitees to inform them about having received permissions to the
    /// OneDrive item
    /// </param>
    ...
    /// Defines how to deal with the scenario where a similarly named file already exists at
    /// the target location
    /// </param>
    /// <param name="cancellationToken">Cancellation Token (optional)</param>
    /// <returns>OneDriveItem representing the uploaded file when successful or NULL when the upload failed</returns>
    ...
    public async Task<OneDriveItem> UploadFileToAppFolderAs(string filePath, string fileName, NameConflictBehavior nameConflictBehavior, CancellationToken cancellationToken = new())
    {
        ...
        return await UploadFileToAppFolderViaResumableUpload(fileToUpload, fileName, null, cancellationToken);
    }
    ...
    /// <summary>
    /// Performs a file upload to the AppFolder on OneDrive using the simple OneDrive API. Best for small files on reliable
    /// network connections.
    /// </summary>
    ...
    {
        return await SearchInternal($"drive/root/search(q='{query}')", cancellationToken);
    }
    ...
    protected async Task<OneDriveUploadSession> CreateResumableUploadSession(string fileName, OneDriveItem oneDriveItem, NameConflictBehavior nameConflictBehavior, CancellationToken cancellationToken = new())
    {
        ...
        else if (!string.IsNullOrEmpty(oneDriveItem.WebUrl) && oneDriveItem.WebUrl.Contains("cid="))
        {
            ...
            completeUrl = $"drives/{oneDriveItem.WebUrl.Remove(0, oneDriveItem.WebUrl.IndexOf("cid=", StringComparison.Ordinal) + 4)}/items/{oneDriveItem.Id}:/{fileName}:/createUploadSession";
        }
        else
        {
    ...
    protected override async Task<OneDriveUploadSession> CreateResumableUploadSession(string fileName, OneDriveItem oneDriveFolder, CancellationToken cancellationToken = new())
    {
        ...
        {
            // Item will be uploaded to another drive. Used by OneDrive Personal when using a shared item.
            completeUrl = $"drives/{oneDriveFolder.WebUrl.Remove(0, oneDriveFolder.WebUrl.IndexOf("cid=", StringComparison.Ordinal) + 4)}/items/{oneDriveFolder.Id}:/{fileName}:/createUploadSession";
        }
        else
        {
            // Item will be uploaded to the current user its drive
    ...
    {
        return await GetGraphData<SharePointSite>("sites/root", cancellationToken);
    }
    ...
    /// </summary>
    /// <param name="siteId">
    /// Unique identifier of the SharePoint site to retrieve, i.e.
    /// tenant.sharepoint.com,42f21fb5-a809-41d6-a97c-64ea0935306f,5a153572-749b-45e8-bae3-4a5e108ffa85
    /// </param>
    /// <param name="cancellationToken">Cancellation Token (optional)</param>
    /// <returns>SharePointSite instance containing the details of the requested site in SharePoint</returns>
